...
for r=1:length(runName)
    ...
    %
    % fgName='run1_lmax8_curvature1_classified_clean'
    % roiName={'lh_IFG_union_morphing_reading_vs_all_sphere_7mm' 'lh_ITG_intersection_math_reading_sphere_7mm'}
    % foi=19
    % fiberCount = fatRoiFiberOverlap(fatDir, sessid, runName(r), fgName, roiName, foi, '5.00')
    %
    % fgName='run1_lmax8_curvature1_classified_clean'
    % roiName={'lh_IPCS_morphing_adding_vs_all_sphere_7mm' 'lh_ITG_intersection_math_reading'}
    % foi=19
    % fiberCount = fatRoiFiberOverlap(fatDir, sessid, runName(r), fgName, roiName, foi, '5.00')
    
    
    % fgName='run1_lmax8_curvature1_classified_clean_AF_overlap'
    % roiName={'lh_IFG_union_morphing_reading_vs_all_sphere_7mm_lh_OTS_union_morphing_reading_vs_all_sphere_7mm' 'lh_IPCS_morphing_adding_vs_all_sphere_7mm_lh_ITG_intersection_math_reading'}
    % foi=19
    %
    % fiberCount = fatRoiFiberOverlap(fatDir, sessid, runName(r), fgName, roiName, foi, '5.00')
    % fiberCount = fatRoiFiberUnique(fatDir, sessid, runName(r), fgName, roiName, foi, '5.00')
    %
    roiName={'lh_ITG_intersection_math_reading_sphere_7mm_lh_IFG_union_morphing_reading_vs_all_sphere_7mm' 'lh_ITG_intersection_math_reading_sphere_7mm_lh_IPCS_morphing_adding_vs_all_sphere_7mm'}
    ...
    for s=1:20
        ...
        if (exist(roifgFile1,'file')>0 && exist(roifgFile2,'file')>0)
            ...
            roiName={'lh_ITG_intersection_math_reading_sphere_7mm_lh_IPCS_morphing_adding_vs_all_sphere_7mm' 'lh_ITG_intersection_math_reading_sphere_7mm_lh_IFG_union_morphing_reading_vs_all_sphere_7mm'}
            fiberCount = fatRoiFiberUnique(fatDir, sessid(s), runName(r), fgName, roiName, foi, '1.00')
        else
            fib(s,:)=0;
        end
