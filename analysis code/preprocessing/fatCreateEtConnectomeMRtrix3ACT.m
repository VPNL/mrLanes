...
function fatCreateConnectomeMRtrix3ACT(fatDir, fsDir, sessid, anatid, runName)
...
rois = fullfile(runDir, '/t1/niftiRois/allROIsProjedMerged.nii.gz');
...
if ET
    ...
    tck_file = fullfile(tractPath, '/fibers/WholeBrainFGRoiSe.tck');
    algo     = 'iFOD1';
    bkgrnd = false;
    verbose = true;
    mrtrixVersion = 3;
    numconcatenate = [];
     for na=1:length(ET_angleValues)
         fgFileName{na}=['ET_fibs_RoiSeeds' num2str(ET_numberFibers) '_angle' strrep(num2str(ET_angleValues(na)),'.','p') '.tck'];
        fgFileNameWithDir{na}=fullfile(tractPath, '/fibers/', fgFileName{na});
        fgFileNameWithDir2=fullfile(tractPath, '/fibers/WholeBrainFGRoiSe.mat');
