function bigImg = fatMontageForPublication(fatDir, sessid, runName, imgName)
...
for r = 1
    ...
    cnt = 1;
    for s = find(idx)
        runDir = fullfile(fatDir,runName{r},'dti96trilin');
        imgDir = fullfile(runDir, 'fibers','afq','image');
...
%     txtImg = export_fig('-native');
    ...
    % write image
    if ~isempty(img)
    imwrite(img,fullfile(fatDir,sprintf('montage_%s',imgName)),'Resolution', 600);
    end
end
