...
for tract=1:3
    ...
    if tract ==1
        pairwiseTracts={'rh_ISMG_morphing_reading_vs_all_projed_gmwmi_rh_IFG_union_morphing_reading_vs_all_projed_gmwmi_r1.00_WholeBrainFGRoiSe_classified_clean_overlap_unique.mat'...
            'rh_ISMG_morphing_adding_vs_all_projed_gmwmi_rh_IPCS_morphing_adding_vs_all_projed_gmwmi_r1.00_WholeBrainFGRoiSe_classified_clean_overlap_unique.mat'};
        outname=strcat('FigS14_fibers_SLF.tif');
        subjects=1:20
        num=16; %refers to the numerical code used in AFQ to label differenct fascicles, 15=left SLF
    elseif tract ==2
        pairwiseTracts={'rh_pSTS_MTG_union_morphing_reading_vs_all_projed_gmwmi_rh_IFG_union_morphing_reading_vs_all_projed_gmwmi_r1.00_WholeBrainFGRoiSe_classified_clean_overlap_unique.mat'...
            'rh_ITG_morphing_adding_projed_gmwmi_rh_IPCS_morphing_adding_vs_all_projed_gmwmi_r1.00_WholeBrainFGRoiSe_classified_clean_overlap_unique.mat'};
        outname=strcat('FigS14_fibers_AF.tif');
        subjects=1:20
        num=20;  %refers to the numerical code used in AFQ to label differenct fascicles, 19=left arcuate
        
    elseif tract ==3 % math
        pairwiseTracts={'rh_ITG_intersection_math_reading_projed_gmwmi_rh_IFG_union_morphing_reading_vs_all_projed_gmwmi_r1.00_WholeBrainFGRoiSe_classified_clean_overlap_unique.mat'...
            'rh_ITG_intersection_math_reading_projed_gmwmi_rh_IPCS_morphing_adding_vs_all_projed_gmwmi_r1.00_WholeBrainFGRoiSe_classified_clean_overlap_unique.mat'};
        num=20;
        subjects=1:20
        outname=strcat('FigS14_fibers_AF_Intersection.tif');
    end
    ...
    roifg=output_roifg;
    if tract==1
        save(fullfile(ExpDir, sessid{s}, '96dir_run1/fw_afq_ET_ACT_LiFE_3.0.2_lmax8/dti96trilin/fibers','afq','rh_SMG_ITG_IPCS_IFG_SLF_overlap.mat'),'roifg');
        ROIfg=['rh_SMG_ITG_IPCS_IFG_SLF_overlap.mat'];
    elseif tract==2
        save(fullfile(ExpDir, sessid{s}, '96dir_run1/fw_afq_ET_ACT_LiFE_3.0.2_lmax8/dti96trilin/fibers','afq','rh_STS_ITG_IPCS_IFG_AF_overlap.mat'),'roifg');
        ROIfg=['rh_STS_ITG_IPCS_IFG_AF_overlap.mat'];
    elseif tract==3
        save(fullfile(ExpDir, sessid{s}, '96dir_run1/fw_afq_ET_ACT_LiFE_3.0.2_lmax8/dti96trilin/fibers','afq','rh_ITG_Inter_IPCS_IFG_AF_overlap.mat'),'roifg');
        ROIfg=['rh_ITG_Inter_IPCS_IFG_AF_overlap.mat'];
    end
    
%     fibersToPlot=[1:2];
%     colors=[0 0.8 0; 0 0 0.8];
%     fatRenderFibersForPublication(ExpDir, sessid{s}, '96dir_run1/fw_afq_ET_ACT_LiFE_3.0.2_lmax8', ROIfg,fibersToPlot,t1name,'rh',colors)
%     cd(OutDir);
   ...
   elseif tract==3
    ...
    if tract==1
        outname=('stat_euclD_SLF.mat');
         outname2=strcat('data_distance_SLF.mat');
    elseif tract==2
        outname=('stat_euclD_AF.mat');
       outname2=strcat('data_distance_AF.mat');
    elseif tract==3
        outname=('stat_euclD_AF_Intersection.mat');
        outname2=strcat('data_distance_AF_Intersection.mat');
    end
    
    save(outname,'kstest_h','kstest_p','ttest_h','ttest_p','ttest_c','ttest_stat')
        cd('data');
    save(outname2,'histmeanWithin','histmeanAcross','EuclDistanceWithin','EuclDistanceAcross','Within','Across');
     cd(OutDir);
%     
...
%     
%     if tract==1
%         outname=strcat('FigS14_decoding_SLF.tif');
%     elseif tract==2
%         outname=strcat('FigS14_decoding_AF.tif');
%     elseif tract==3
%         outname=strcat('FigS14_decoding_AF_intersection.tif');
%     end
%     
